...
add_filter('woocommerce_breadcrumb_defaults', 'kale_woocommerce_breadcrumbs');
function kale_woocommerce_breadcrumbs()
{
    return array(
        'delimiter' => '',
...
add_action('woocommerce_before_shop_loop_item', 'kale_template_loop_product_thumb', 10);
add_action('woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_add_to_cart', 10);
add_action('woocommerce_before_shop_loop_item_title', 'kale_template_loop_product_thumb_close', 10);
add_action('woocommerce_shop_loop_item_title', 'kale_template_loop_product_title', 10);
...
function kale_template_loop_product_thumb()
{
...
function kale_template_loop_product_thumb_close()
{
    //global $woocommerce;
    //echo "\t\t<a href=\"" . $woocommerce->cart->get_cart_url() . "\" class=\"added_to_cart\" title=\"" . esc_html__( 'View cart', 'kale' ) . "\">" . esc_html__( 'View cart', 'kale' ) . "</a>\n";
    echo "\t\t</div>\n";
}

#Show the product title in the product loop. By default this is an H2.

function kale_template_loop_product_title()
{
...
function woocommerce_get_product_thumbnail($size = 'large', $deprecated1 = 0, $deprecated2 = 0)
{
    ...
    if (has_post_thumbnail()) {
        ...
        return '<img src="'.woocommerce_placeholder_img_src().'" alt="'.esc_html__('No image', 'kale').'" class="img-responsive" />';
    }
}
    ...
    function woocommerce_form_field( $key, $args, $value = null ) {
        ...
        if ( $args['required'] ) {
            ...
            //$required = ' <span class="asterisk" title="' . esc_attr__( 'required', 'kale' ) . '">*</span>';
			$required = ' <abbr class="required" title="' . esc_attr__( 'required', 'kale' ) . '">*</abbr>';
        } else {
            $required = '';
        }
        ...
        switch ( $args['type'] ) {
            case 'country' :
                ...
                if ( 1 === sizeof( $countries ) ) {
                    ...
                    $field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select form-control selectpicker ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '>' . '<option value="">' . esc_html__( 'Select a country&hellip;', 'kale' ) . '</option>';
                    ...
                    $field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__( 'Update country', 'kale' ) . '" /></noscript>';
            ...
            case 'state' :
                ...
                if ( is_array( $states ) && empty( $states ) ) {
                    ...
                    $field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select form-control selectpicker' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
						<option value="">' . esc_html__( 'Select a state&hellip;', 'kale' ) . '</option>';
                    ...
                    foreach ( $args['options'] as $option_key => $option_text ) {
                        if ( '' === $option_key ) {
                            ...
                            if ( empty( $args['placeholder'] ) ) {
                                $args['placeholder'] = $option_text ? $option_text : __( 'Choose an option', 'kale' );
                            }
...
add_filter("woocommerce_checkout_fields", "kale_order_fields");

function kale_order_fields($fields) {
...
function kale_is_active_woocommerce_sidebar(){
	if(is_shop()){
