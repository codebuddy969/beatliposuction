...
<div class="full-width-recipe-index">
    ...
    <?php 
	$kale_page_meta = get_post_meta(get_the_ID(),'_recipe_index_meta',TRUE);
	$kale_categories = explode(',', $kale_page_meta['categories']);
    if(count($kale_categories)>0) {
        if(!empty($kale_page_meta['number_of_posts']))  $kale_n = $kale_page_meta['number_of_posts'];
        else $kale_n = 3;
        $kale_n = absint($kale_n); 
    } else {
    ...
    <div class="recipe-index-feed">
        ...
        <?php $kale_the_content = get_the_content(); if(!empty($kale_the_content)) { ?>
